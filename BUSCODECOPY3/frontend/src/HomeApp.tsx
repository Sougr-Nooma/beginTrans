...
type ViewMode = 'SEARCH' | 'BOOKING' | 'TICKET' | 'RESERVATION_SUCCESS' | 'COMPANIES' | 'MY_BOOKINGS' | 'ABOUT' | 'RECRUITMENT' | 'OFFERS' | 'SUPPORT';

interface HomeAppProps {
  mode?: 'public' | 'client';
}

export default function HomeApp({ mode = 'public' }: HomeAppProps) {
  ...
  const popularCompanies = companies.slice(0, 3);
  ...
  return (
    <div className="min-h-screen bg-[#FDFCFB] font-sans text-gray-900">
      <Navbar 
        variant={mode === 'client' ? 'client' : 'public'}
        currentUser={mode === 'client' ? currentUser : null}
        onLogout={() => {
          logout();
          navigate('/auth');
        }}
      ...
      <main className="pt-16">
        ...
        {error && !loading && (
          <div className="max-w-lg mx-auto my-12 p-6 bg-brand-red/5 border border-brand-red/20 rounded-2xl text-center text-brand-red font-semibold">
            {error}
            <button type="button" onClick={() => refresh()} className="block mx-auto mt-4 text-sm underline">
              Réessayer
            </button>
          </div>
        )}
